...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          frm_ALERTA.Memo1.lines.Add('ATENÇÃO: O cancelamento desta venda somente    ');
          frm_ALERTA.Memo1.lines.Add('         pode ser efetuada pelos COMANDOS ECF! ');
          frm_ALERTA.Memo1.lines.Add('         (e somente a venda do último cupom)   ');
      ...
      begin
        ...
        begin
                     ...
                     //*** Consulta o código do cliente e o seu contrato ************
                     //*** OBS: Criado para a situação de multiboleta ***************
          ...
          if (cancelouMulti) then
            if (doc3 <> 0) then
              frmDialogo.ExibirMensagem ('Também foram canceladas as vendas '+floattostr(doc2)+' e '+floattostr(doc3)+' pois pertencem a um contrato multiboleta!',
                'Cancelamento de Venda',[mbOk],frm_principal.x_pathimg+'iconeerro.bmp',
                250,200)
            else
              frmDialogo.ExibirMensagem ('Também foi cancelada a venda '+floattostr(doc2)+' pois pertence a um contrato multiboleta!',
                'Cancelamento de Venda',[mbOk],frm_principal.x_pathimg+'iconeerro.bmp',
                250,200);
        end;
      end;
  ...
  if (Modo=1) then //TROCA DE VENDEDOR
    ...
    if (ds1.dataset.fieldbyname('VE_FUNC').AsFloat<>frm_principal.x_caixa_ativo) then
      ExibeCritica ('A Venda selecionada não foi lançada pelo OPERADOR DE CAIXA ativo no momento!','Troca de vendedor',nil)
    else
    begin
