...
type
  Tfrm_aux1Contrato_demo = class(TForm)
    btnProc: TButton;
    MainMenu1: TMainMenu;
    Label2: TLabel;
    edtLoja: TMaskEdit;
...
var
  frm_aux1Contrato_demo: Tfrm_aux1Contrato_demo;

implementation

uses cadcli, principal, loja, F8Lojas, funcoes1, funcoes2, f8Clientes,
  unDialogo, cadcli2, aux1Contrato;

{$R *.DFM}


{Ativacao do formulario}
procedure Tfrm_aux1Contrato_demo.FormActivate(Sender: TObject);
begin
  if (frm_edtCliente2.cdLoja<>0) then
    edtLoja.text := form_nz(frm_edtCliente2.cdLoja,3);
  if (frm_edtCliente2.datacontrato<>StrToDate('01/01/1900')) then
    edtDataContrato.text := DateToStr(frm_edtCliente2.datacontrato)
  else
    edtDataContrato.text:=DateToStr(frm_principal.x_data_trabalho);
  if (frm_edtCliente2.cdCliente<>0) then
    edtCliente.text := form_nz(frm_edtCliente2.cdCliente,6);
  edtLoja.setfocus;
  edtCliente.setfocus;
  btnProc.setfocus;
  if (frm_edtCliente2.Modo=0) then
  begin
    ...
    btnProc.top:=112;
    btnCancel.top:=112;
    frm_aux1Contrato_demo.height:=196;
    pnInfo1.visible:=true;
    pnInfo2.visible:=false;
    bevel1.height:=136;
  end
  else
  if (frm_edtCliente2.Modo=2) then
  begin
    label3.visible:=true;
    ...
    btnProc.top:=176;
    btnProc.top:=176;
    frm_aux1Contrato_demo.height:=290;
    pnInfo1.visible:=true;
...
procedure Tfrm_aux1Contrato_demo.SAIR1Click(Sender: TObject);
begin
  close;
end;

{Navegacao-}
procedure Tfrm_aux1Contrato_demo.btnProcClick(Sender: TObject);
var
  continua: Boolean;
begin
     {CRITICA AS DATAS}
  ...
  begin
    ...
    begin
      if (Trim(frm_aux1Contrato_demo.edtLoja.text)<>'') then
        cdLoja := StrToInt(Trim(frm_aux1Contrato_demo.edtLoja.text))
      else
        cdLoja := 0;
      frm_edtCliente2.edtLoja.text:=form_nz(cdLoja,3);
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtLojaEnter(Sender: TObject);
begin
  edtLoja.selectall;
end;

{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtLojaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtDataContratoEnter(Sender: TObject);
begin
  edtDataContrato.selectall;
end;

{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtDataContratoKeyDown(Sender: TObject;
...
procedure Tfrm_aux1Contrato_demo.edtLojaExit(Sender: TObject);
var
  clLoja: TClassLoja;
  codigo: String;
begin
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.btnCancelClick(Sender: TObject);
begin
  close;
end;

{Navegacao-}
procedure Tfrm_aux1Contrato_demo.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtDataContratoExit(Sender: TObject);
begin
...
procedure Tfrm_aux1Contrato_demo.edtLojaDblClick(Sender: TObject);
begin
  with (frm_f8Lojas) do
  begin
    left:=287;
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtClienteKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var
  clClie: TClassCliente;
  codigo: String;
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtClienteExit(Sender: TObject);
var
  clClie: TClassCliente;
  codigo: String;
begin
...
{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtClienteEnter(Sender: TObject);
begin
  edtCliente.selectall;
end;

{Navegacao-}
procedure Tfrm_aux1Contrato_demo.edtClienteDblClick(Sender: TObject);
begin
  with (frm_f8Clientes) do
  begin
    edit:=edtCliente;
