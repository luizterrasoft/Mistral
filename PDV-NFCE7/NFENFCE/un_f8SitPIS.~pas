...
procedure Tfrm_F8SitPIS.botao_sairClick(Sender: TObject);
begin
  close;
end;

procedure Tfrm_F8SitPIS.gradeDblClick(Sender: TObject);
begin
...
procedure Tfrm_F8SitPIS.gradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_F8SitPIS.edt_DescKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     {teclando ENTER}
  if (key=K_ENTER) then
...
procedure Tfrm_F8SitPIS.botaoProcurarClick(Sender: TObject);
begin
{ INICIO }
...
{ MEU CODIGO - LUIZ CAPINZAL - SC, 15.10.2015 20:52 }
  DMNFE.qCSTPISF8.Close;
  DMNFE.qCSTPISF8.SQL.Clear;
  DMNFE.qCSTPISF8.SQL.Add('SELECT * FROM CST_PIS');
  DMNFE.qCSTPISF8.SQL.Add('WHERE descricao LIKE :desc');
  DMNFE.qCSTPISF8.ParamByName('desc').AsString := '%' + edt_Desc.Text + '%';
  DMNFE.qCSTPISF8.Active := True;
  DMNFE.qCSTPISF8.Open;
end;
