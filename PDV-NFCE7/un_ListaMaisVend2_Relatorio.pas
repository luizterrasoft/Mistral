...
uses
  ...
  ExtCtrls, QuickRpt, Qrctrls, Db, DBTables,funcoesglobais;
...
type
  Tfrm_ListaMaisVend2_Relatorio = class(TForm)
    ...
    TotalGeral: TQRLabel;
    PercqtdeProduto: TQRLabel;
    PercValorProduto: TQRLabel;
    qVendas1RefCodigo: TStringField;
    qVendas1Referencia: TStringField;
    qVendas1RefForn: TStringField;
...
uses principal, funcoes1, funcoes2, venda, procura, un_ListaMaisVend2;
...
begin
  ...
  with Tquery.create(Application) do
    try
      databasename := QVendas1.DataBaseName;
      sql.text := substituicamposquery(TQuery(Relatorio.DataSet).Text,'SUM(IV_TOTA/IV_PVEN) as QTDE,SUM(IV_TOTA) as Valor');
      if LikeStrings(Sql,'Group') > - 1 then
        sql.delete(LikeStrings(Sql,'Group'));
      if LikeStrings(Sql,'Order') > - 1 then
        sql.delete(LikeStrings(Sql,'Order'));
      Params.Assign(TQuery(Relatorio.DataSet).Params);
      open;

      PercQtdeProduto.caption      := formatfloat('#,##0.00',y_total_qtdeProduto/fieldbyname('Qtde').AsFloat * 100);
      PercValorProduto.caption     := formatfloat('#,##0.00',y_total_ValorProduto/fieldbyname('Valor').AsFloat * 100);
      QtdeGeral.caption            := fieldbyname('Qtde').AsString;
      TotalGeral.caption           := formatfloat('#,##0.00',fieldbyname('Valor').AsFloat);
    finally
      destroy;
    end;
     {......................................................}
