...
begin
  ...
  if frm_principal.MOD_NFE = 1 then
     ...
     ShowMessage('ESTE MÓDULO DE NFE NÃO ESTA LIBERADO PARA SUA EMPRESA!');
end;
 ...
 if (ServicoSeFazAtivo() = 'OK') Then
    Begin
      if frm_principal.MOD_NFCE = 1 then
         ...
         ShowMessage('ESTE MÓDULO DE NFCE NÃO ESTA LIBERADO PARA SUA EMPRESA!');
    end
  else
         ShowMessage('Serviço SEFAZ / INTERNET - INATIVO(S) - FAVOR VERIFICAR');
 end;
...
begin
  ...
  begin
    ...
    frm_VendaDeVales.caption  := 'EMISSÃO DE VALES AVULSOS';
...
begin
  ...
  begin
    frmDialogo.ExibirMensagem ('Consulta de Itens de Estoque das Lojas não Habilitada'
      ,'Venda',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200);
    exit;                           
  end;
