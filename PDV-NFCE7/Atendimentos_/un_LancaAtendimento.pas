...
begin
  ...
  if (frm_principal.x_vendedor=0) then
    frmDialogo.ExibirMensagem ('Vendedor indefinido! Lançamento do item não permitido!'
      ,'Atendimentos',[mbOK]
...
begin
  if (frm_principal.x_vendedor=0) then
    frmDialogo.ExibirMensagem ('Vendedor indefinido! Busca por produto não permitida!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200)
  else
  begin
...
begin
  {***************************************************************************************}
  if (y_qtde_itens_distintos=0) then
    frmDialogo.ExibirMensagem ('Não foram informados itens para     o atendimento!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200)
  else
...
begin
  if (n_cor<>-1) then
    ...
    desc_cor := '';
  if (desc_cor='<Cor não cadastrada>') then
  begin
...
begin
  ...
  pnSit.caption               := ' Situação: ATDO. ABERTO';
  pnLoja.caption              := ' Loja: '+form_nz(frm_principal.x_loja,2)+'/'+form_t(ProcuraNomeLoja(frm_principal.x_loja,self),20);
  pnCaixa.caption             := ' Nº Terminal: '+form_nz(frm_principal.x_terminal,2);
     {********************************************************************************}
  lblSeq.caption              := 'Último Atdo.: '+floattostr(frm_principal.x_codigo_atendimento);
end;
...
begin
  ...
  begin
    ...
    frm_ALERTA.Memo1.lines.Add('A não configuração deste código causará erros de inconsistência nos ATENDIMENTOS.');
  ...
  begin
    frm_ALERTA.Memo1.lines.clear;
    frm_ALERTA.Memo1.lines.Add('Erro: Código do terminal de atdo. não configurado! ');
    frm_ALERTA.Memo1.lines.Add('A não configuração deste código causará erros de inconsistência nos ATENDIMENTOS.');
    frm_ALERTA.Memo1.lines.Add(' - Entre em contato com o GERENTE da LOJA para configurar este terminal -');
    frm_ALERTA.showmodal;
    ...
    begin
      ...
      frm_principal.pnCaixa.caption := ' Nº Terminal: '+form_nz(frm_principal.x_terminal,2);
      if (frm_principal.x_terminal=0) then
  ...
  begin
    frm_ALERTA.Memo1.lines.clear;
    frm_ALERTA.Memo1.lines.Add('Erro: O código do último atdo. não pode ser menor que ');
    frm_ALERTA.Memo1.lines.Add('      que o último atdo. lançado! ');
    frm_ALERTA.showmodal;
...
begin
  ...
  if (JaTeveTroca) or (JaTeveVenda) or (JaTeveDevolucao) then
    frmDialogo.ExibirMensagem ('O vendedor não pode ser trocado enquanto houver atendimento em andamento!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200)
  else
  begin
...
begin
  ...
  begin
    frmDialogo.ExibirMensagem ('Não foram informados itens para     o atendimento!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200);
    frmDialogo.ExibirMensagem ('Suspensão impossível!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200)
  end
  else
  if (frmDialogo.ExibirMensagem ('Confirma suspensão do Atendimento?'
    ,'Atendimentos',[mbYes,mbNo]
    ,frm_principal.x_pathimg+'iconequestion.bmp',250,200)=mrYes) then
  begin
              //****************************************************
...
begin
  ...
  if (y_qtde_itens_distintos<>0) then
  begin
    frmDialogo.ExibirMensagem ('Já existem itens informados neste       Atendimento!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200);
    frmDialogo.ExibirMensagem ('Impossível reabrir outro Atendimento!'
      ,'Atendimentos',[mbOK]
      ,frm_principal.x_pathimg+'iconeerro.bmp',250,200);
  end
  else
