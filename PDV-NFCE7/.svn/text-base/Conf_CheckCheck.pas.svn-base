...
var
  ...
  begin
    ...
    for i:=0 to FStringsC.Count - 1 do
    begin
      if Retorno.Cells[1,1] <> '' then
        Retorno.RowCount := Retorno.rowcount + 1;

      Query1.First;
      if Query1.Locate('Ck_Grupo;Ck_Campo',VarArrayOf([Fgrupo,FStringsC[i]]),[]) then
      begin
...
begin
  ...
  begin
    sql.clear;
    sql.add('Select Ck_Pos,Ck_Consulta,Ck_Valor,Ck_FaixaIni,Ck_Tempo,ck_senhat,Ck_cliente,Ck_Senha,Ck_Serial,ck_bloq,ck_senhat,ck_senhaM From CheckCheck');
  ...
  begin
    ...
    MovimentaCelula := true;
    with setcombo(ColunaCampo('Ck_Exibe'),-1,-1)do
    begin
      Items.add('Sim');
      Items.add('Não');
    end;
    with setcombo(ColunaCampo('Ck_Trava'),-1,-1)do
    begin
      Items.add('Sim');
      Items.add('Não');
      Items.add('Senha');
    ...
    with setcombo(ColunaCampo('Ck_Alinhamento'),-1,-1)do
    begin
      Items.add('Esquerda');
      Items.add('Direita');
    end;
...
begin
  if (uppercase(dadosgrid.field) = 'CK_POS') then
  begin
    DadosGrid.SemMascara := false;
    DadosGrid.Mascara    := '000';
  end
  else
  if (uppercase(dadosgrid.field) <> 'CK_VALOR') and (uppercase(dadosgrid.field) <> 'CK_FAIXAINI') and (uppercase(dadosgrid.field) <> 'CK_FAIXAFIM') then
...
begin
  ...
  try
    ...
    query1.sql.clear;
    query1.sql.add('Insert Into CheckCheck (Ck_Pos,Ck_Consulta,Ck_Valor,Ck_FaixaIni,Ck_Tempo,ck_senhat,Ck_cliente,Ck_Senha,Ck_Serial,ck_bloq,ck_senhaM)');
    query1.sql.add('Values (:Ck_Pos,:Ck_Consulta,:Ck_Valor,:Ck_FaixaIni,:Ck_Tempo,:ck_senhat,:Ck_cliente,:Ck_Senha,:Ck_Serial,:ck_bloq,:ck_senhaM)');
...
//       Query1.ParamByName('Ck_SenhaM').AsString  := SenhaM.text;
    ...
    for i:=1 to GridCons.RowCount - 1 do
      if gridCons.fieldvalue['Ck_Consulta',I] <> '' then
      begin
//            Query1.ParamByName('Ck_SenhaM').AsString := SenhaM.text;
    ...
    for i:=1 to Retorno.RowCount - 1 do
    begin
      Query1.ParamByName('Ck_Pos').AsInteger := I;
      if (Retorno.fieldvalue['Ck_Trava',I] <> '') or
        (Retorno.fieldvalue['Ck_Exibe',I] <> '') then
      begin
        for i2:=Retorno.ColunaCampo('Ck_Exibe') to Retorno.ColunaCampo('Ck_Campo') do
          if UpperCase(Retorno.campos[i2]) = 'CK_TAM' then
            Query1.ParamByName(Retorno.campos[i2]).AsInteger := StrtoIntN(Retorno.Cells[i2,I])
          else
