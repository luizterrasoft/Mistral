...
begin
  ...
  begin
    y_caixa := strtofloat(Trim(edtNumero2.text));
          {*************************************************************}
    clSeq := TClassAuxiliar.Create;
    clSeq.conect   ('C:\NATIVA',self);
    clSeq.ClearSql;
    clSeq.AddParam ('UPDATE CONF SET CODIGO_CAIXA=:codigo_caixa ');
    clSeq.consulta.parambyname('codigo_caixa').AsFloat := y_caixa;
    clSeq.Execute;
    clSeq.desconect;
    clSeq.Free;
          {*************************************************************}
...
begin
  ...
  edtNumero1.text := floattostr(y_caixa);
  edtNumero2.text := floattostr(y_caixa);
  edtNumero2.setfocus;
end;
