...
type
   ...
   TTipo   = (cdEAN13,cdEAN11,cdEAN8,cdUPC,cd2de5,cd3x9);
...
function  DefineEAN11 (CodigoC:string):string;
...
Procedure Register;
begin
   RegisterComponents('Kernel',[TABarra,TABarraPrinter]);
end;
...
begin
   ...
   if Tipo = cdEAN11  then CodigoExt := DefineEAN11  (Codigo);
...
var
   ...
   Codifi  : string;
   I       : integer;
   Loop,Tamanho,Modulo,Total,Valor,nDigito : Word;
   Resto : Integer;
begin
   Result := 'Erro';
   for I := 1 to length(CodigoC) do
   ...
   T2de5[0] := '100100100100'; {0}
   T2de5[1] := '100100100110'; {1}
   T2de5[2] := '100100110100'; {2}
   T2de5[3] := '100100110110'; {3}
   T2de5[4] := '100110100100'; {4}
   T2de5[5] := '100110100110'; {5}
   T2de5[6] := '100110110100'; {6}
   T2de5[7] := '100110110110'; {7}
   T2de5[8] := '110100100100'; {8}
   T2de5[9] := '110100100110'; {9}

   //Digi := 0;

   //if (length(CodigoC) = 11) then
   //  Begin
   //      DigitoPEG   := CodigoC[11];
   //      Digi        := -1;
   //      CodigoC     := Copy(CodigoC,1,10);
   //  End;

       if (length(CodigoC) = 10) then
       begin
         Tamanho := Length(CodigoC);
         Modulo  := 2;
         Total   := 0;
         For Loop := Tamanho DownTo 1 Do Begin
            If StrToInt(CodigoC[Loop]) > 0 Then Begin
                 Valor := StrToInt(CodigoC[Loop]);
                 Total := Total + (Valor * Modulo);
            End;
            Inc(Modulo);
            If Modulo > 9 Then
                Modulo := 2;
         ...
         Resto := Total Mod 11;
         Resto := Resto - 11 ;
         nDigito := Abs(Resto) Mod 10;
         DigitoPEG := IntToStr(nDigito)[1];

         Codifi := '';
         CodigoC := CodigoC + DigitoPEG;
         ...
         begin
             if (CodigoC[I] <> '0') and
                ...
                Codifi := Codifi + T2de5[StrToInt(CodigoC[I])];
         end;
         { Caracter Stop }
         Result := Codifi;

         //Result := CodigoC + IntToStr(nDigito)[1];
