...
type
  ...
  TFrm_ConsCheckCheck = class(TForm)
    ...
    Label3: TLabel;
    Label4: TLabel;
    BConsulta: TBitBtnControle;
    BotaoVoltar: TSpeedButton;
    BotaoSeguir: TSpeedButton;
    Cmc7B: TEditControle;
    Cmc7C: TEditControle;
    Label5: TLabel;
    EditControle1: TEditControle;
    EditControle2: TEditControle;
    Label6: TLabel;
    Label7: TLabel;
...
//    procedure FClickBotao(Sender: TObject);
    ...
    procedure BotaoSeguirClick(Sender: TObject);
  private
...
begin
  ...
  try
    with Frm_ConsCheckCheck do begin
      ...
      if (Frm_Principal.X_CheckCheckProtocolo = 'ISO8583 EXTENDED' ) then
        iProtocolo := 2
      else if( Frm_Principal.X_CheckCheckProtocolo = 'ISO8583' ) then
        iProtocolo := 1
      else
        iProtocolo := 0;
      ...
      else if( iTipoServidor = 1 ) then   //Servidor PAC
      begin
     ...
     //Abrindo conexao
      ...
      i := 1;
{marcio      While not Query1.eof do begin
        With TBitBtnControle.Create(Frm_ConsCheckCheck) do begin
          Parent  := Panel2;
...
//          OnClick := FClickBotao;
          ...
          if Pos('CheckCadastro',Caption) > 0 Then
           Tag := 5
          else if Pos('CheckCheck',Caption) > 0  Then
           Tag := 55
          else if Pos('CheckPlus',Caption) > 0 Then
           Tag := 56
          else if Pos('CheckMaster',Caption) > 0 Then
           Tag := 57
          else if Pos('CheckGold',Caption) > 0 Then
           Tag := 58;
...
begin
 ...
 for i:=0 to Length(FGruposLogin) - 1 do
  if FGruposLogin[i] = Grupo then begin
    Result := Mlogin;
    Exit;;
  end;

 for i:=0 to Length(FGruposCheques) - 1 do
  if FGruposCheques[i] = Grupo then begin
    Result := Cheques;
    Exit;;
  end;

 for i:=0 to Length(FGruposOutras) - 1 do
  if FGruposOutras[i] = Grupo then begin
    Result := Outras;
    Exit;;
  end;

 for i:=0 to Length(FGruposPen) - 1 do
  if FGruposPen[i] = Grupo then begin
    Result := Pendencias;
    Exit;;
  end;

 for i:=0 to Length(FGruposCliente) - 1 do
  if FGruposCliente[i] = Grupo then begin
    Result := Cliente;
    Exit;;
  end;
...
begin
  ...
  for i:=0 to Length(FGruposReg) - 1 do
   if uppercase(FgruposReg[i]) = UpperCase(Fgrupo) then begin
     Result := True;
     exit;
   end;
end;
...
if TBitBtn(Sender).Tag = 55 then
begin
  if StrToFloatN(Trim(Valor.Text)) = 0 then
  begin
     frmDialogo.ExibirMensagem ('Preenchimento obrigatório!'
                ,'Consulta Check Check',[mbOk],
                frm_principal.x_pathimg+'iconeerro.bmp',250,200);
     Valor.SetFocus;
     cont := False;
  end
...
if TBitBtn(Sender).Tag = 56 then
begin
  if ((StrToFloatN(Trim(Valor.Text))) = 0) or (Trim(Banco.Text)='') or ((Trim(Agencia.Text)='')) or
     (Trim(ChequeIni.Text)='') or (Trim(ChequeFinal.Text)='') or (Trim(Conta.Text)='') then
  begin
     frmDialogo.ExibirMensagem ('Preenchimento obrigatório!'
                ,'Consulta Check Check',[mbOk],
                frm_principal.x_pathimg+'iconeerro.bmp',250,200);
     if (StrToFloatN(Trim(Valor.Text))=0) then Valor.SetFocus
     else if (Trim(Banco.Text))='' then Banco.SetFocus
     else if (Trim(Agencia.Text))='' then Agencia.SetFocus
     else if (Trim(Conta.Text))='' then Conta.SetFocus
     else if (Trim(ChequeIni.Text))='' then ChequeIni.SetFocus
     else if (Trim(ChequeFinal.Text))='' then ChequeFinal.SetFocus;
     cont := False;
  end
  else cont := True;
end;
if (TBitBtn(Sender).Tag = 57) or (TBitBtn(Sender).Tag = 58) then
begin
  if StrToFloatN(Trim(Valor.Text)) = 0 then
  begin
     frmDialogo.ExibirMensagem ('Preenchimento obrigatório!'
...
//Marcio---------------------------------------------------------------------------
      ...
      begin
         ...
         if (frm_senhaSupervisor.y_flag) then continua := True else continua := False;
      end
      else continua := True;
      ...
      else if iTransacao = 55 Then    //ChkRequestCheckCheck
      begin
      ...
      else if iTransacao = 56 Then    //ChkRequestCheckPlus
      begin
      ...
      else if iTransacao = 57 Then    //ChkRequestCheckMaster
      begin
      ...
      else if iTransacao = 58 Then    //ChkRequestCheckGold
      begin
      ...
      begin
        ...
        ShowMessage(marcio);
        if FClassCheckCheck.FGrupo.indexof('ERR') >= 0 then begin
        ...
        While Not Query1.Eof do begin
          ...
          with Fmemo do begin
            if FClassCheckCheck.Fgrupo.IndexOf(Query1.FieldByName('Ck_grupo').AsString) = -1 then begin
              query1.Next;
              continue;
            end;
            ...
            FirstCampo := '';
            FLinha := 0;
            While (FClassCheckCheck.FCampo.Count > Ind + I) and (Copy(FClassCheckCheck.FCampo[Ind + LikeStrings(FClassCheckCheck.FCampo,Query1.FieldByName('Ck_Grupo').AsString+'*')],1,4) = Query1.FieldByName('Ck_Grupo').AsString+'*') do begin
              query2.first;
              ...
              if not Query2.Locate('CK_CAMPO',F,[])  then begin
               Inc(Ind);
               Continue;
              end;
              ...
              if gruporeg(Query1.FieldByName('Ck_Grupo').AsString) then
                if firstcampo = '' then
                  FirstCampo := FValor
                else if (Fvalor <> '') and (FirstCampo = FValor) then begin
                  Lines.Add('');
                  Inc(FLinha);
                end;

              fvalor := FClassCheckCheck.RetornaValor(Query1.FieldByName('Ck_Grupo').AsString,Query2.FieldByName('Ck_Campo').AsString,Flinha);
              if gruporeg(Query1.FieldByName('Ck_Grupo').AsString) then begin
                 if (Query2.FieldByName('Ck_Alinhamento').AsString = 'Direita') then begin
                  try
                    FValor := formatfloat('#,##0.00',StrtofloatN(FValor));
                  Except
                   //
                  end;
                   Lines[Lines.Count - 1] := Lines[Lines.Count - 1] +iifs(Lines[Lines.Count - 1] <> '',' ','') + PadL(RetornaNomeGrupoCheckCheck(Query1.FieldByName('Ck_Grupo').AsString,Query2.FieldByName('Ck_campo').AsString),20,'.')+': '+PadL(FValor,Query2.FieldByName('Ck_Tam').AsInteger,' ');
                 end else
                   Lines[Lines.Count - 1] := Lines[Lines.Count - 1] +iifs(Lines[Lines.Count - 1] <> '',' ','') + PadR(RetornaNomeGrupoCheckCheck(Query1.FieldByName('Ck_Grupo').AsString,Query2.FieldByName('Ck_campo').AsString),20,'.')+': '+PadR(FValor,Query2.FieldByName('Ck_Tam').AsInteger,' ');
              end else
...
{marcio                 FFocu := true;
        ...
        if LikeStrings(FClassCheckCheck.Fgrupo,'NCO0') > -1   then begin
         if Query1.FieldByName('Ck_Bloq').AsString = 'T' then
           LbCheckCheck.Color   := Clred;
           PiscaLabel1.Caption := 'Cliente com pendências financeira';
           LbCheckCheck.Caption := 'Cliente com pendências financeira de acordo com a CheckCheck';
        end else if LikeStrings(FClassCheckCheck.Fgrupo,'NCO1') > -1   then begin
         LbCheckCheck.Color   := ClYellow;
         if Query1.FieldByName('Ck_Bloq').AsString = 'T' then
           PiscaLabel1.Caption := 'Cliente com observações,mas sem pendências financeiras';
         LbCheckCheck.Caption := 'Cliente com observações,mas sem pendências financeiras de acordo com a CheckCheck';
        end else begin
        ...
        Obs.Lines.Clear;
        FSenha := False;
        If (Query1.FieldByName('Ck_Bloq').AsString <> 'T') and (Query1.FieldByName('Ck_Bloq').AsString <> 'S') then begin
          ...
          with TQuery.Create(Application) do
           try
             ...
             while not Eof do begin
               ...
               if (FClassCheckCheck.FGrupo.indexof(Fgrupo) >= 0) Then begin
            ...
            for i:=0 to Length(FgruposRest) - 1 do
             if (FClassCheckCheck.FGrupo.indexof(FgruposRest[i]) >= 0) Then Begin
        ...
        else if Obs.Lines.Text <> ''   then begin
           PiscaLabel1.Color   := Clred;
           PiscaLabel1.Caption := 'Cliente com pendências financeira';
           LbCheckCheck.Color   := Clred;
           LbCheckCheck.Caption := 'Cliente com pendências financeira';
        end else begin
       ...
       With Query2 do begin
        ...
        With Query1 do begin
             ...
             ParamByName('Ck_Usuario').AsString    := frm_principal.lbl_usuario.caption;
...
begin
   ...
   BotaoSeguir.Visible := False;
   if frm_Consulta1 <> nil then if frm_edtCliente = nil then CnpjCpf.Text := frm_Consulta1.cpfcnpj;
end;
...
begin
  if Not(Length(CnpjCpf.Text) in [11,14]) or
    ...
    ((Length(CnpjCpf.Text) = 14) and not CHEK_CGC(CnpjCpf.Text)) then begin
     frmDialogo.ExibirMensagem (' Cpf/Cnpj Inválido'
     ,'Consulta CheckCheck',[mbOK]
     ,frm_principal.x_pathimg+'iconeerro.bmp',250,200);
     CnpjCpf.SetFocus;
     exit;
  end;
...
//  FCampo.Add('');
  Result := '';
  i := 1;
  While length(FBuffer) >= i do begin
    if (FBuffer[i] = '<') then begin
       ...
       if (FBuffer[i] = '|') then
        I := i + 1;
       continue;
    end else if (FBuffer[i] = '>') or (FBuffer[i] = ',') or (FBuffer[i] = '|') then begin
       if (FBuffer[i] <> '>') tHEN
        Fcampo.Add(Fgrupo[FGrupo.count - 1]+'*');
       inc(i);
       Continue;
...
begin
    ...
    if trim(campo) = '' Then begin
      if uppercase(grupo) = 'LOI' Then
        Result := 'Login'
      else If uppercase(grupo) = 'MSG' Then
        Result := 'Mensagem de Propaganda'
      else If uppercase(grupo) = 'FRN' Then
        Result := 'Dados da Franquia'
      else if uppercase(grupo) = 'LAS' Then
        Result := 'Lista de serviços'
      else if uppercase(grupo) = 'ERR' Then
        Result := 'Erros'
      else If uppercase(grupo) = 'FRN' Then
        Result := 'Lista de consultas permitidas'
      else If uppercase(grupo) = 'FRN' Then
        Result := 'Erro na Transação'
      else If uppercase(grupo) = 'NOM' Then
        Result := 'Nomes relacionados ao CNPJ/CPF'
      else If uppercase(grupo) = 'CFM' Then
        Result := 'Confirmação de Nome na Receita Federal'
      else If uppercase(grupo) = 'BCO' Then
        Result := 'Situação do Banco perante ao CCO'
      else If uppercase(grupo) = 'AGE' Then
        Result := 'Endereço de agência bancária'
      else If uppercase(grupo) = 'RCH' Then
        Result := 'Recheque online'
      else If uppercase(grupo) = 'CFV' Then
        Result := 'Cheques sem fundos varejo'
      else If uppercase(grupo) = 'PVJ' Then
        Result := 'Pendências internas no varejo'
      else If uppercase(grupo) = 'NCO' Then
        Result := 'Consta alguma restrição'
      else If uppercase(grupo) = 'NC1' Then
        Result := 'Nada Consta'
      else If uppercase(grupo) = 'NC2' Then
        Result := 'Constam somente informações complementares'
      else If uppercase(grupo) = 'CCF' Then
        Result := 'Cheques sem Fundos'
      else If uppercase(grupo) = 'RCO' Then
        Result := 'Restrições Comerciais.'
      else If uppercase(grupo) = 'TLB' Then
        Result := 'Telebloqueio para o cheque consultado'
      else If uppercase(grupo) = 'TBC' Then
        Result := 'Telebloqueio para outros cheques não consultados'
      else If uppercase(grupo) = 'PLU' Then
        Result := 'Cheques Contra Ordenados'
      else If uppercase(grupo) = 'SEM' Then
        Result := 'Cheques Contra Ordenados com dados semelhantes'
      else If uppercase(grupo) = 'CHI' Then
        Result := 'Cheques Irregulares'
      else If uppercase(grupo) = 'PRT' Then
        Result := 'Protestos'
      else If uppercase(grupo) = 'ACO' Then
        Result := 'Ações'
      else If uppercase(grupo) = 'FAL' Then
        Result := 'Falências'
      else If uppercase(grupo) = 'RFN' Then
        Result := 'Refinanciamentos'
      else If uppercase(grupo) = 'CTM' Then
        Result := 'Contumácia de Cheques contra-ordenados'
      else If uppercase(grupo) = 'PFN' Then
        Result := 'Pendências Financeiras'
      else If uppercase(grupo) = 'INS' Then
        Result := 'Insucessos Empresariais'
      else If uppercase(grupo) = 'TEL' Then
        Result := 'Telefones Confirmados'
      else If uppercase(grupo) = 'VCL' Then
        Result := 'Veiculos Retornados'
      else If uppercase(grupo) = 'CRE' Then
        Result := 'Código da Resposta'
      else If uppercase(grupo) = 'DHC' Then
        Result := 'Data e Hora da consulta'
      else If uppercase(grupo) = 'RST' Then
        Result := 'Histórico das últimas consultas';
      exit;
    end;

    if uppercase(grupo) = 'LOI' Then begin
      if campo = 'STATUS_LOGIN' then
       result := 'Status do Login'
      else if campo = 'COD_RESP' then
       Result := 'Código de Resposta do Login'
      else if campo = 'MENS' then
      ...
      else if campo = 'RAZAO' then
       Result := 'Razão Social'
      else if campo = 'END' then
       Result := 'Endereço'
      else if campo = 'BAIRRO' then
       Result := 'Bairro'
      else if campo = 'CIDADE' then
       Result := 'Cidade'
      else if campo = 'UF' then
       Result := 'UF'
      else if campo = 'CEP' then
       Result := 'Cep'
      else if campo = 'EMAIL' then
       Result := 'Email'
      else if campo = 'FONE' then
       Result := 'Fone'
      else if campo = 'FRANQ' then
       Result := 'Franquia'
      else if campo = 'USO_CMC7' then
       Result := 'Usa CMC7'
      else if campo = 'TIME_CONS' then
       Result := 'TimeOut de Consultas'
      else if campo = 'TIME_LOGIN' then
       Result := 'TimeOut de LOGIN';
    end else if uppercase(grupo) = 'MSG' Then begin
      if campo = 'TIPO_MSG' then
       result := 'Tipo de mensagem'
      else if campo = 'MENSAGEM' then
       Result := 'Mensagem';
    end else if uppercase(grupo) = 'FRN' Then begin
      if campo = 'frn_cod' then
       result := 'Codigo'
      else if campo = 'FRN_RSO' then
       Result := 'Razão Social'
      else if campo = 'FRN_TEL' then
       Result := 'Tel'
      else if campo = 'FRN_FAX' then
       Result := 'Fax'
      else if campo = 'FRN_END' then
       Result := 'Endereço'
      else if campo = 'FRN_BAI' then
       Result := 'Bairro'
      else if campo = 'FRN_CID' then
       Result := 'Cidade'
      else if campo = 'FRN_UF' then
       Result := 'UF'
      else if campo = 'FRN_CEP' then
       Result := 'Cep'
      else if campo = 'FRN_EMAIL' then
       Result := 'Email';
    end else if uppercase(grupo) = 'LAS' Then begin
      if campo = 'PCF_FAIXA' then
       result := 'Perfil'
      else if campo = 'COD_SRV' then
       Result := 'Codigo'
      else if campo = 'SRV_DEF' then
       Result := 'Default'
      else if campo = 'VAL_MIN' then
       Result := 'Min'
      else if campo = 'VAL_MAX' then
       Result := 'Max';
    end else if uppercase(grupo) = 'ERR' Then begin
      if campo = 'NIV_ERRO' then
       result := 'Nível'
      else if campo = 'MSG_COD_ERR' then
       Result := 'Mensagem';
    end else if uppercase(grupo) = 'NOM' Then begin
      if campo = 'NOME' then
       result := 'Nome';
    end else if uppercase(grupo) = 'CFM' Then begin
      if campo = 'NOME' then
       result := 'Nome'
      else if campo = 'DATANASC' then
       Result := 'Data de Nascimento'
      else if campo = 'SITREC' then
       Result := 'Sit. Cpf/Cnpj'
      else if campo = 'DATAAT' then
       Result := 'Atualização'
      else if campo = 'NOMEMAE' then
       Result := 'Nome da Mae';
    end else if uppercase(grupo) = 'BCO' Then begin
      if campo = 'QUANT' then
       result := 'Quant'
      else if campo = 'NOME' then
       Result := 'Nome'
       ...
       Result := 'Descrição';
    end else if uppercase(grupo) = 'AGE' Then begin
      if campo = 'QUANT' then
       result := 'Quant'
      else if campo = 'NOME' then
       Result := 'Nome'
       ...
       Result := 'UF'
      else if campo = 'DATAULTATUALIZACAO' then
       Result := 'Atualização'
      else if campo = 'DDD' then
       Result := 'DDD'
      else if campo = 'FONE' then
       Result := 'FONE'
      else if campo = 'DDDFAX' then
       Result := 'DDDFAX'
      else if campo = 'FAX' then
       Result := 'FAX'
    end else if uppercase(grupo) = 'RCH' Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'SIT' then
        result := 'Rec'
       else if result = 'DATAABERTENCERR' then
        result := 'Abertura/Encerramento'
       else if result = 'TIPOPESSOA' then
        result := 'Pessoa'
       else if result = 'CPFCNPJ' then
        result := 'Cpf/Cnpj'
       else if result = 'NOME' then
        result := 'Nome'
       else if result = 'TIPODOC' then
        result := 'Tipo Doc.'
       else if result = 'DOC' then
        result := 'Nº'
       else if result = 'EMISSOR' then
        result := 'Og.Emissor'
       else if result = 'UF' then
        result := 'UF'
       else if result = 'DATAEMISSAO' then
        result := 'Emissao';
    end else if uppercase(grupo) = 'CFV' Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DATAREC' then
        result := 'Data'
       else if result = 'CODCOMP' then
        result := 'Cod.Compensação'
       else if result = 'NOME' then
        result := 'Nome'
       else if result = 'NUMOC' then
        result := 'Nº Ocorrencia'
       else if result = 'ORIGEM' then
        result := 'Origem'
       else if result = 'SIGLA' then
        result := 'Sigla/Embratel'
       else if result = 'NUMLOJA' then
        result := 'Nº da Loja';
    end else if uppercase(grupo) = 'PVJ' Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DATAREC' then
        result := 'Data'
        ...
        result := 'Nº da Loja';
    end else if uppercase(grupo) = 'CCF' Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DATA_ANT' then
        result := 'Data do primeiro cheque'
       else if result = 'DATA_REC' then
        result := 'Data do cheque mais recente'
       else if result = 'COD_BNC' then
        result := 'Banco'
       else if result = 'NOM_BNC' then
        result := 'Nome'
       else if result = 'COD_AGE' then
        result := 'Agencia'
       else if result = 'NOM_AGE' then
        result := 'Nome'
       else if result = 'DATA_INC_REC' then
        result := 'Data Inc.'
       else if result = 'DATA_PROC_PRI_INC' then
        result := 'Data Primeiro Proc'
       else if result = 'TIP_PES_ASSOC' then
        result := 'Pessoa'
       else if result = 'ALN_BNC' then
        result := 'Alinea do Banco'
    end else if uppercase(grupo) = 'RCO' Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'EMP_NOM' then
        result := 'Empresa'
       else if result = 'EMP_TEL' then
        result := 'Telefone'
       else if result = 'DAT_DEB' then
        result := 'Ocorrencia'
       else if result = 'DAT_INC' then
        result := 'Inclusão'
       else if result = 'COD_MOT_INC' then
        result := 'Codigo Motivo'
       else if result = 'COD_BNC' then
        result := 'Banco'
       else if result = 'COD_AGE' then
        result := 'Age'
       else if result = 'NUM_CHQ' then
        result := 'Cheque'
       else if result = 'ALN' then
        result := 'Alinea'
       else if result = 'NUM_PARC' then
        result := 'Nº Parcelas '
       else if result = 'TOT_PARC' then
        result := 'Total '
       else if result = 'NUM_MEM' then
        result := 'Mensalidades'
       else if result = 'NUM_DUP' then
        result := 'Duplicatas'
       else if result = 'EMP_UF' then
        result := 'Uf'
       else if result = 'MOT_INC_CLI' then
        result := 'Motivo'
       else if result = 'EMP_RAZ' then
        result := 'Empresa'
       else if result = 'CNPJ' then
        result := 'CNPJ'
       else if result = 'TIP_PES' then
        result := 'Pessoa'
       else if result = 'OCR_VAL' then
        result := 'Valor'
    end else if (uppercase(grupo) = 'TLB') or (uppercase(grupo) = 'TBC')  Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'COD_BNC' then
        result := 'Banco'
        ...
        result := 'Nome'
       else if result = 'CNT_COR' then
        result := 'Conta'
       else if result = 'CHQ_INI' then
        result := 'Cheque Ini'
       else if result = 'CHQ_FIM' then
        result := 'Cheque Fim'
       else if result = 'MOTIV' then
        result := 'Motivo'
       else if result = 'DAT_REG' then
        result := 'Data'
       else if result = 'HORA' then
        result := 'Hora';
    end else if (uppercase(grupo) = 'PLU') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'COD_BNC' then
        result := 'Banco'
        ...
        result := 'Cheque Fim'
       else if result = 'DATA_CCO' then
        result := 'Data'
       else if result = 'TIP_CCO' then
        result := 'Tipo'
       else if result = 'HORA' then
        result := 'Hora'
       else if result = 'FONTE' then
        result := 'Fonte';
    end else if (uppercase(grupo) = 'CTM') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'COD_BNC' then
        result := 'Banco'
        ...
        result := 'Fonte';
    end else if (uppercase(grupo) = 'SEM') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'COD_BNC' then
        result := 'Banco'
        ...
        result := 'Hora'
       else if result = 'DOC_ORI' then
        result := 'Doc';
    end else if (uppercase(grupo) = 'PFN') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
        result := 'Data da 1ª Pend'
       else if result = 'DAT_REC' then
        result := 'Data da Ult. Pend'
       else if result = 'PFN_DATA' then
        result := 'Data'
       else if result = 'PFN_MOD' then
        result := 'Modalidade'
       else if result = 'AVAL' then
        result := 'Avalista'
       else if result = 'TIPO_MOEDA' then
        result := 'Moeda'
       else if result = 'PFN_VAL' then
        result := 'Valor'
       else if result = 'CONTR' then
        result := 'Contrato'
       else if result = 'PFN_ORI' then
        result := 'Origem'
       else if result = 'SIGLA' then
        result := 'Sigla'
    end else if (uppercase(grupo) = 'RFN') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
        result := 'Data da 1ª Pend'
        ...
        result := 'Data da Ult. Pend'
       else if result = 'RFN_DATA' then
        result := 'Data'
       else if result = 'RFN_NAT' then
        result := 'Natureza'
       else if result = 'COD_BNC' then
        result := 'Banco'
       else if result = 'COD_AGE' then
        result := 'Age'
       else if result = 'RFN_VAL' then
        result := 'Valor'
       else if result = 'UF' then
        result := 'UF'
       else if result = 'NOM_BNC' then
        result := 'Nome Banco'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'AVAL' then
        result := 'Avalista';
    end else if (uppercase(grupo) = 'CHI') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'COD_BNC' then
        result := 'Banco'
        ...
        result := 'Aline'
       else if result = 'CHQ_VAL' then
        result := 'Valor'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'UF' then
        result := 'UF';
    end else if (uppercase(grupo) = 'INS') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
       ...
       else if result = 'INS_DATA' then
        result := 'Data'
       else if result = 'INS_QLF' then
        result := 'Qualf.'
       else if result = 'INS_VAR' then
        result := 'Vara'
       else if result = 'NOM_PES' then
        result := 'Nome';
    end else if (uppercase(grupo) = 'PRT') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
       ...
       else if result = 'PRT_DATA' then
        result := 'Data'
       else if result = 'COD_CART' then
        result := 'Cartório'
       else if result = 'PRT_NAT' then
        result := 'Natureza'
       else if result = 'PRT_VAL' then
        result := 'Valor'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'UF' then
        result := 'UF';
    end else if (uppercase(grupo) = 'ACO') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
       ...
       else if result = 'ACO_DAT' then
        result := 'Data'
       else if result = 'COD_VAR' then
        result := 'Vara'
       else if result = 'COD_DIST' then
        result := 'Distribuidor'
       else if result = 'ACO_NATU' then
        result := 'Natureza'
       else if result = 'ACO_VAL' then
        result := 'Valor'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'UF' then
        result := 'UF'
       else if result = 'PRI_COO' then
        result := 'Status';
    end else if (uppercase(grupo) = 'FAL') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DAT_ANT' then
       ...
       else if result = 'FAL_DAT' then
        result := 'Data'
       else if result = 'FAL_VAR' then
        result := 'Vara'
       else if result = 'FAL_NATU' then
        result := 'Natureza'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'UF' then
        result := 'UF';
    end else if (uppercase(grupo) = 'TEL') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'INFO' then
        result := 'Info'
       else if result = 'NOME' then
        result := 'Nome'
       else if result = 'TIP_PES' then
        result := 'Pessoa'
       else if result = 'CPF_CNPJ' then
        result := 'Cpf/Cnpj'
       else if result = 'END' then
        result := 'Endereço'
       else if result = 'BAI' then
        result := 'Bairro'
       else if result = 'CID' then
        result := 'Cidade'
       else if result = 'UF' then
        result := 'UF'
       else if result = 'CEP' then
        result := 'CEP'
       else if result = 'DDD' then
        result := 'DDD'
       else if result = 'FONE' then
        result := 'FONE';
    end else if (uppercase(grupo) = 'VCL') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'RENAVAM' then
        result := 'Renavam'
       else if result = 'PLACA' then
        result := 'Placa'
       else if result = 'UF' then
        result := 'UF'
       else if result = 'CHASSI' then
        result := 'Chassi'
       else if result = 'CIDADE' then
        result := 'Cidade'
       else if result = 'TIP_MONT' then
        result := 'Montagem'
       else if result = 'TIP_VEIC' then
        result := 'Tipo'
       else if result = 'MARCA' then
        result := 'Marca'
       else if result = 'ESPECIE' then
        result := 'Especie'
       else if result = 'SITUACAO' then
        result := 'Situação'
       else if result = 'COR' then
        result := 'Cor'
       else if result = 'ANO' then
        result := 'Ano'
       else if result = 'COMBUSTIVEL' then
        result := 'Combustivel'
       else if result = 'PROCED' then
        result := 'Procedencia'
       else if result = 'TIP_DOC_PROP' then
        result := 'PESSOA'
       else if result = 'DOC_PROP' then
        result := 'Cpf/Cnpj Propietario'
       else if result = 'NOME_PROP' then
        result := 'Propietario'
       else if result = 'DATA_ULT_ATUA' then
        result := 'Atualização'
       else if result = 'CILIND' then
        result := 'Cilindradas'
       else if result = 'POTENCIA' then
        result := 'Potencia'
       else if result = 'MOTOR' then
        result := 'Motor'
       else if result = 'QTD_EIXOS' then
        result := 'Eixos'
       else if result = 'REST1' then
        result := 'Arrendamento'
       else if result = 'REST2' then
        result := 'Alienação'
       else if result = 'REST3' then
        result := 'Roubo'
       else if result = 'REST4' then
        result := 'Judicial'
       else if result = 'MAX_PASS' then
        result := 'Passageiros'
       else if result = 'PESO_BRUTO' then
        result := 'Peso'
       else if result = 'ANO_FAB' then
        result := 'Ano'
       else if result = 'TIP_CHASSI' then
        result := 'Tipo chassi'
       else if result = 'REST5' then
        result := 'Administrativas'
       else if result = 'REST6' then
        result := 'Reserva Dominio'
       else if result = 'REST7' then
        result := 'Tributarias'
    end else if (uppercase(grupo) = 'RST') Then begin
       if campo = 'QUANT' then
        result := 'Quant'
       else if result = 'DATA' then
        result := 'Data'
       else if result = 'RAZAO' then
        result := 'Razao'
       else if result = 'TIP_PES' then
        result := 'Pessoa'
       else if result = 'CPF_CNPJ' then
        ...
        result := 'Nome';
    end else if (uppercase(grupo) = 'DHC') Then
...
begin
 try
  ...
  if campo < 0 Then begin
    ...
    if uppercase(grupo) = 'LOI' Then begin
    ...
    end else if uppercase(grupo) = 'LOI' Then begin
      case campo of
       0:Result := 'STATUS_LOGIN';
       1:Result := 'cod_resp';
       2:Result := 'mens';
      ...
      case campo of
       ...
       Result := acertaretornosarray(['nome','DataNasc','SitRec','DataAt','NomeMae'],campo);
      end;
    end else if uppercase(grupo) = 'BCO' Then begin
      case campo of
       0:Result := 'quant';
      ...
      case campo of
       ...
       Result := acertaretornosarray(['DataAnt','Datarec','DataOc','Mod','Aval','Moeda','Valor'
      ...
      case campo of
       ...
       Result := acertaretornosarray(['dat_ant','dat_rec','cod_bnc','nom_bnc','cod_age','nom_age'
      ...
      case campo of
       ...
       Result := acertaretornosarray(['emp_nom','emp_tel','dat_deb','dat_inc',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['cod_bnc','nom_bnc','cod_age','nom_age',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['pfn_dat','pfn_mod',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['rfn_dat','rfn_nat',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['chi_dat','cod_bnc',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['ins_dat','ins_nat','ins_qlf','ins_var','nom_pes'],campo);
      end;
    end else if uppercase(grupo) = 'PRT' Then begin
      case campo of
       0:Result := 'quant';
       ...
       Result := acertaretornosarray(['prt_dat','cod_cart','prt_nat','prt_val','cid','uf'],campo);
      end;
    end else if uppercase(grupo) = 'ACO' Then begin
      case campo of
       0:Result := 'quant';
       ...
       Result := acertaretornosarray(['aco_dat','cod_var','cod_dist','aco_nat','aco_val','cid','uf','pri_coo'],campo);
      end;
    end else if uppercase(grupo) = 'FAL' Then begin
      case campo of
       0:Result := 'quant';
       ...
       Result := acertaretornosarray(['fal_dat','fal_nat','fal_var','cid','uf'],campo);
      end;
    end else if uppercase(grupo) = 'TEL' Then begin
      case campo of
       0:Result := 'quant';
      else
       Result := acertaretornosarray(['info','nome','tip_pes','cpf_cnpj','end'
      ...
      case campo of
       ...
       Result := acertaretornosarray(['renavam','placa','uf','chassi','cidade','tip_mont',
      ...
      case campo of
       ...
       Result := acertaretornosarray(['data','razao','cidade','uf','fone','nome'],campo);
      end;
    end else if uppercase(grupo) = 'DHC' Then begin
      case campo of
       0:Result := 'dat_hor';
      end;
    end else if uppercase(grupo) = 'CRE' Then begin
      case campo of
       0:Result := 'cod_resp';
      end;
    end;
...
{     If SemQuant  and (UpperCase(Result) = 'QUANT') then
...
begin
  ...
  If Result <> '' Then
    ...
    else if (Pos('DAT_',UpperCase(Campo)) = 1) or (Pos('DATA',UpperCase(Campo)) = 1) Then
      Result := Copy(Result,7,2)+'/'+Copy(Result,5,2)+'/'+Copy(Result,1,4)
    else if (Pos('CPF',UpperCase(Campo)) > 0) or  (Pos('CNPJ',UpperCase(Campo)) > 0) then
...
//   BConfirma.OnClick(BConfirma);
  if key  = vk_escape then
   close;
  if Key = vk_F5 then
   BConsulta.OnClick(BConsulta);

{marcio  if (Shift = [ssctrl]) and
     (key>=Ord('1')) and (key<=Ord('9')) then begin
     flist := TList.Create;
...
begin
  ...
  if piscalabel1.visible and (piscalabel1.color <> clred) then begin
    BConfirma.ModalResult := MrOk;
    close;
    exit;
  end;

  if FSenha then begin
    frm_senha_checkcheck := tfrm_senha_checkcheck.create(application);
    try
      frm_senha_checkcheck.showmodal;
      if frm_senha_checkcheck.BtnAplicar.ModalResult = MrOk then begin
        BConfirma.ModalResult := MrOk;
        close;
        With Query1 do begin
             sql.clear;
...
procedure TFrm_ConsCheckCheck.BConsultaClick(Sender: TObject);
var iTRansacao:integer;
    FLinha,ind,i:integer;
    F,FValor,oldgrupo : string;
    fmemo:tmemo;
...
qUERY1.Sql.Clear;
query1.Sql.Add('Select * from CheckCheck');
Query1.SQL.Add('Where Ck_FaixaIni <= :faixaini and Ck_FaixaFim >= :faixafim');
Query1.ParamByName('faixaini').AsFloat := StrToFloatN(Valor.Text);
Query1.ParamByName('faixafim').AsFloat := StrToFloatN(Valor.Text);
Query1.Open;

if Query1.RecordCount <= 0 then begin iTRansacao := 5; nomecons := 'CheckCadastro' end;

if Query1.FieldByName('Ck_Consulta').AsString = 'CheckCadastro' then begin
   iTRansacao := 5; nomecons := 'CheckCadastro'
end
else if Query1.FieldByName('Ck_Consulta').AsString = 'CheckCheck' then begin
   iTRansacao := 55; nomecons := 'CheckCheck'
end
else if Query1.FieldByName('Ck_Consulta').AsString = 'CheckPlus' then begin
   iTRansacao := 56; nomecons := 'CheckPlus'
end
else if Query1.FieldByName('Ck_Consulta').AsString = 'CheckMaster' then begin
   iTRansacao := 57; nomecons := 'CheckMaster'
end
else if Query1.FieldByName('Ck_Consulta').AsString = 'CheckGold' then begin
   iTRansacao := 58; nomecons := 'CheckGold'
end;
...
begin
  if (Trim(Cmc7A.Text) = '') then
     begin
       if (Trim(Banco.Text)='') or ((Trim(Agencia.Text)='')) or
          (Trim(ChequeIni.Text)='') or (Trim(ChequeFinal.Text)='') or (Trim(Conta.Text)='') then
       begin
            frmDialogo.ExibirMensagem ('Preenchimento obrigatório!'
                  ,'Consulta Check Check',[mbOk],
                  frm_principal.x_pathimg+'iconeerro.bmp',250,200);
             if (Trim(Banco.Text))='' then Banco.SetFocus
             else if (Trim(Agencia.Text))='' then Agencia.SetFocus
             else if (Trim(Conta.Text))='' then Conta.SetFocus
             else if (Trim(ChequeIni.Text))='' then ChequeIni.SetFocus
             else if (Trim(ChequeFinal.Text))='' then ChequeFinal.SetFocus;
...
if (iTRansacao = 57) or (iTRansacao = 58) then
begin
  if StrToFloatN(Trim(Valor.Text)) = 0 then
  begin
     frmDialogo.ExibirMensagem ('Preenchimento obrigatório!'
...
//Marcio---------------------------------------------------------------------------
      ...
      begin
        ...
        ChkRequestCheckPlus(rq).ChequeInicial := ChequeIni.Text;
        ChkRequestCheckPlus(rq).ChequeFinal := ChequeFinal.Text;
        ChkRequestCheckPlus(rq).Cmc7 := Cmc7A.Text+Cmc7B.Text+Cmc7C.Text;
        ChkRequestCheckPlus(rq).QuantidadeCmc7 := StrToIntN( Qtde.Text);
      ...
      begin
        ...
        ChkRequestCheckMaster(rq).ChequeInicial := ChequeIni.Text;
        ChkRequestCheckMaster(rq).ChequeFinal := ChequeFinal.Text;
        ChkRequestCheckMaster(rq).Cmc7 := CMC7A.Text+Cmc7B.Text+Cmc7C.Text;
        ChkRequestCheckMaster(rq).QuantidadeCmc7 := StrToIntN( Qtde.Text );
      ...
      begin
        ...
        ChkRequestCheckGold(rq).ChequeInicial := ChequeIni.Text;
        ChkRequestCheckGold(rq).ChequeFinal := ChequeFinal.Text;
        ChkRequestCheckGold(rq).Cmc7 := Cmc7A.Text+Cmc7B.Text+Cmc7C.Text;
        ChkRequestCheckGold(rq).QuantidadeCmc7 := StrToIntN( Qtde.Text );
...
//             end;
       ...
       With Query2 do begin
         Sql.Clear;
         Sql.Add('Select Ck_Valor from CheckCheck');
         sql.add('Where Ck_Consulta = '''+nomecons+'''');
         Open;
       end;
        ...
        With Query1 do begin
             ...
             sql.add('Values(:Ck_Loja,:Ck_Consulta,:Ck_Data,:Ck_Hora,:Ck_Numero,:Ck_Cpf,:Ck_Valor,:Ck_Usuario,:Ck_Bloq )');
             ParamByName('Ck_Loja').AsFloat        :=  frm_principal.x_frente_loja;
             ParamByName('Ck_Consulta').AsString   := nomecons;
             ParamByName('Ck_Data').AsDateTime     := StrtoDateN(FormatdateTime('dd/mm/yy',FHoraCons));
             ParamByName('Ck_Hora').AsDateTime     := FHoraCons;
             try
               ParamByName('Ck_Numero').AsFloat      := StrtoFloatN(FClassCheckCheck.retornaValor('CRE','COD_RESP'));
...
procedure TFrm_ConsCheckCheck.PageControl2Enter(Sender: TObject);
begin
  BotaoSeguir.Visible := True;
  BotaoVoltar.Visible := True;
end;

procedure TFrm_ConsCheckCheck.BotaoVoltarClick(Sender: TObject);
begin
if (PageControl2.ActivePage = TabSheet5) then
begin
  if (TabSheet8.TabVisible) then PageControl2.ActivePage := TabSheet8
  else if (TabSheet9.TabVisible) then PageControl2.ActivePage := TabSheet9
  else if (TabSheet7.TabVisible) then PageControl2.ActivePage := TabSheet7
  else if (TabSheet6.TabVisible) then begin PageControl2.ActivePage := TabSheet6; BotaoVoltar.Enabled := False; end;
end
else
if (PageControl2.ActivePage = TabSheet8) then
begin
  if (TabSheet9.TabVisible) then PageControl2.ActivePage := TabSheet9
  else if (TabSheet7.TabVisible) then PageControl2.ActivePage := TabSheet7
  else if (TabSheet6.TabVisible) then begin PageControl2.ActivePage := TabSheet6; BotaoVoltar.Enabled := False; end;
end
else
...
begin
  if (TabSheet7.TabVisible) then PageControl2.ActivePage := TabSheet7
  else if (TabSheet6.TabVisible) then begin PageControl2.ActivePage := TabSheet6; BotaoVoltar.Enabled := False; end;
end
else
if (PageControl2.ActivePage = TabSheet7) then
begin
  if TabSheet6.TabVisible then begin PageControl2.ActivePage := TabSheet6; BotaoVoltar.Enabled := False; end;
end;
end;

procedure TFrm_ConsCheckCheck.PageControl2Exit(Sender: TObject);
begin
 BotaoVoltar.Visible := False;
 BotaoSeguir.Visible := False;
end;

procedure TFrm_ConsCheckCheck.BotaoSeguirClick(Sender: TObject);
begin
if (PageControl2.ActivePage = TabSheet6) then
begin
  if (TabSheet7.TabVisible) then PageControl2.ActivePage := TabSheet7
  else if (TabSheet9.TabVisible) then PageControl2.ActivePage := TabSheet9
  else if (TabSheet8.TabVisible) then PageControl2.ActivePage := TabSheet8
  else if (TabSheet5.TabVisible) then PageControl2.ActivePage := TabSheet5;
end
else
if (PageControl2.ActivePage = TabSheet7) then
begin
...
if (PageControl2.ActivePage = TabSheet8) then
begin
  if (TabSheet5.TabVisible) then PageControl2.ActivePage := TabSheet5;
end;
end;
