...
begin
   if not duplicado then
    ...
    ShowMessage(Assunto+' já existente' +#13+
...
Function  NomeOrigem(Origem:String):String;
begin
  If UpperCase(Origem) =   'ORC' Then  Result  := 'Orçamento';
  ...
  If UpperCase(Origem) =   'PDV' Then  Result  := 'PDV';
  If UpperCase(Origem) =   'DEV' Then  Result  := 'Devolução';
  If UpperCase(Origem) =   'TRF' Then  Result  := 'Transferencia';
  If UpperCase(Origem) =   'NTS' Then  Result  := 'Serviço';
end;
...
begin
  DecodeDate(data,Ano,Mes,Dia);//decodifica a data
   ...
   for i:= 1 to repeticao do // define a quantidade de dias que será diminuido
    begin
       dia := dia + 1;
       if (mes = 1) or (Mes = 3) or (mes = 5)   or (mes = 7) or
        ...
        if mes = 2 then//fim de fevereiro,verifica se é um ano bissexto
          begin
            if dia >= 28 then
             begin
               if ano mod 4 <> 0 then
  ...
  if mes > 9 then//coloca  um zero nas variáveis se for preciso
    mes2 := inttostr(mes)
  else
    mes2 := '0'+inttostr(mes);

  if dia > 9 then//coloca  um zero nas variáveis se for preciso
    dia2 := inttostr(dia)
  else
    dia2 := '0'+inttostr(dia);

  result := dia2+'/'+mes2+'/'+inttostr(ano);// o resultado virá nesse formato de data (dd/mm/yyyy)
end;
   ...
   If DataValidaDM <> 0 Then Begin
       If ((MENSAGEMDATEM = 1) or ((MENSAGEMDATEM = 2) AND (DataValidaDMNao <> Date()))) and 
          ...
          (StrtoTime(TimeToStr(HoraMLocal)) >= time())
           then Begin
             MENSAGEMDATEM := MensOpcoes('O SISTEMA DETECTOU UMA ALTERAÇÃO NO DIA DA MAQUINA'+#13+
        ...
        While Not FPesquisa2.eof do begin
         ...
         If Not Result.Valida then
           If ((Result.QtdeUtilizada > Result.Quantidade) and (Result.Quantidade > 0)) Then
                Result.Mensagem   :=  IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM A QUANTIDADE DO LIMITE DE CRÉDITO ULTRAPASSADA '+#13+#10+
                                      'QUANTIDADE LIMITE'+PadL(FloatToStr(Result.Quantidade),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM A QUANTIDADE DO LIMITE DE CRÉDITO ULTRAPASSADA') * 2 - 19,' ')+#13+#10+
                                      'QUANTIDADE EM ABERTO'+PadL(FloatToStr(Result.QtdeUtilizada),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM A QUANTIDADE DO LIMITE DE CRÉDITO ULTRAPASSADA')  * 2 - 27,' ')+#13+#10+
                                      'DIFERENÇA'+PadL(FloattoStr(Result.QtdeUtilizada - Result.Quantidade),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM A QUANTIDADE DO LIMITE DE CRÉDITO ULTRAPASSADA') * 2 - 27,' ')
           else if ((Result.Total > 0) and (Result.Utilizado > Result.Total)) then
                Result.Mensagem   :=  IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM LIMITE DE CRÉDITO ULTRAPASSADO '+#13+#10+
                                      'LIMITE'+PadL(FormatFloat('#,##0.00',Result.Total),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM LIMITE DE CRÉDITO ULTRAPASSADO') * 2 - 19,' ')+#13+#10+
                                      'EM ABERTO'+PadL(FormatFloat('#,##0.00',Result.Utilizado),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM LIMITE DE CRÉDITO ULTRAPASSADO')  * 2 - 27,' ')+#13+#10+
                                      'DIFERENÇA'+PadL(FormatFloat('#,##0.00',Result.Utilizado - Result.Total),Length(IIfs(PagRec = 'R','CLIENTE','FORNECEDOR')+' COM LIMITE DE CRÉDITO ULTRAPASSADO') * 2 - 27,' ');
...
begin
    ...
    If Not Result then
       If MensConf('Não foi possível copiar o arquivo '+#13+Cfrom+' para '+Cto+#13+
                  'Deseja Repetir a Cópia ?') then
    ...
    With FrmOpcao do begin
         ...
         if tempoerro <= 0 then
           ...
           while tempoerro  >= GetTickCount - FStart  do begin
              Label1.caption := 'Tempo restante para confirmar a opção selecionada '+FormatDateTime('hh:mm:ss',(TempoErro - (GetTickCount -  Fstart))/Msecsperday)+' s';
              Application.ProcessMessages;
...
begin
  ...
  for i:=1 to length(texto) do
   begin
     ...
     Else if letra = 'É' then letra := 'E'
     Else if letra = 'Í' then letra := 'I'
     Else if letra = 'Ó' then letra := 'O'
     Else if letra = 'Ú' then letra := 'U'
     Else if letra = 'Ã' then letra := 'A'
     Else if letra = 'Õ' then letra := 'O'
     Else if letra = 'À' then letra := 'A'
     Else if letra = 'È' then letra := 'E'
     Else if letra = 'Ì' then letra := 'I'
     Else if letra = 'Ò' then letra := 'O'
     Else if letra = 'Ù' then letra := 'U'
     Else if letra = 'Ü' then letra := 'U'
     Else if letra = 'Ç' then letra := 'C'
     Else if letra = '€' then letra := 'C'
     Else if letra = 'á' then letra := 'a'
     Else if letra = 'é' then letra := 'e'
     Else if letra = 'í' then letra := 'i'
     Else if letra = 'ó' then letra := 'o'
     Else if letra = 'ú' then letra := 'u'
...
begin
   ...
   if not InputQuery('Informe a descrição','Informe a Descrição',FValue) then begin
     exit;
   end;
