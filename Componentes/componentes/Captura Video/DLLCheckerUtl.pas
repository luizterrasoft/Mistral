{ The procedures in this unit was initially in
  Theo Bebekis's zip32.pas and/or unzip32.pas.
  Since the procedures are duplicate somehow,
  I decide to move them into this new unit.
  It is noted that however that the owner of
  copyright of the code is not me and remains
...
-----------------------------------------------------------------------------------------
 ...
 0.03      02.09.2003    Moved Get_FileVersionInfo into this unit.       Siqing Tang
                         since it was duplicate in zip32.pas and
                         unzip32.pas.
-----------------------------------------------------------------------------------------
*)
...
function IsExpectedDllVersion(const PDLLName{no path}, PDLLVersion, PCompanyName{'': don't care}: string): Boolean;
...
{----------------------------------------------------------------------------------
 ...
 Author         : Theo Bebekis <bebekis@otenet.gr>
                  ...
                  Changed by Siqing Tang to take two parameters
                  September 2003, Siqing
-----------------------------------------------------------------------------------}
function Get_FileVersionInfo(const FullPath: string; SubBlock: TFVISubBlock; var sValue: string):boolean;
const
...
begin
  iRes := SearchPath(nil,            { address of search path }
                    PChar(PDLLName), { address of filename }
  ...
  if iRes = 0 then raise Exception.CreateFmt(DLL_WARNING, [PDLLName]);

  Result := Get_FileVersionInfo(String(pBuffer), sbFileVersion, sVersion)
      and (sVersion = PDLLVersion);
  if PCompanyName <> '' then begin
    Result := Result
        and Get_FileVersionInfo(String(pBuffer), sbCompanyName, sCompany)
        and (sCompany = PCompanyName);
  end;
  if not Result then raise Exception.CreateFmt(DLL_VERSION_WARNING, [PDLLName]);
end;
