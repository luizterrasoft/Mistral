...
uses
   Windows, classes, SysUtils, Graphics, extctrls, NMSMTP,Dialogs,Printers,Quickrpt,Forms,QRPrntr, Db, DBTables,FuncPrint,QrpPrev
   {$IFDEF VER140}
...
type
   ...
   published
      ...
      property CodigoSistema : string read fCodigoSistema write FCodigoSistema;
   ...
   private
      ...
      FIntrucroesCobranca: TStringList;
   ...
   public
      ...
      function GetInstrucoesCobranca: TStringList; {Gera a imagem da ficha de compensação}
   published
...
begin
   ...
   AInstrucoes.AddStrings(GetInstrucoesCobranca);
...
function TgbTitulo.GetInstrucoesCobranca: TStringList;
begin
   If Result = Nil then
   ...
   if ValorDespesaCobranca <> 0 then
      Result.Add('Será Cobrada a quantia de  ' + FormatCurr('R$ #,##0.00',ValorDespesacobranca) + 'por despesas bancárias ' );

   if DataProtesto <> 0 then
      Result.Add('Protestar em ' + FormatDateTime('dd/mm/yyyy',DataProtesto));

   if ValorAbatimento <> 0 then
      if DataAbatimento <> 0 then
         Result.Add('Conceder abatimento de ' + FormatCurr('R$ #,##0.00',ValorAbatimento) + ' para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataAbatimento))
      else
         Result.Add('Conceder abatimento de ' + FormatCurr('R$ #,##0.00',ValorAbatimento) + ' para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataVencimento));


   if ValorDesconto <> 0 then
      if DataDesconto <> 0 then
         Result.Add('Conceder desconto de ' + FormatCurr('R$ #,##0.00',ValorDesconto) + ' por dia de antecipação para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataDesconto))
      else
         Result.Add('Conceder desconto de ' + FormatCurr('R$ #,##0.00',ValorDesconto) + ' por dia de antecipação');

   if ValorMulta <> 0 then
      Result.Add('Acrescer o valor do documento em  ' + FormatCurr('R$ #,##0.00',ValorMulta) + ' após o vencimento');

   if ValorMoraJuros <> 0 then
      if DataMoraJuros <> 0 then
         Result.Add('Cobrar juros de ' + FormatCurr('R$ #,##0.00',ValorMoraJuros) + ' por dia de atraso para pagamento a partir de ' + FormatDateTime('dd/mm/yyyy',DataMoraJuros))
      else
         Result.Add('Cobrar juros de ' + FormatCurr('R$ #,##0.00',ValorMoraJuros) + ' por dia de atraso');
end;
...
begin
  for i:=0 to Titulos.Count - 1 do begin
     TBoleto.Create(Owner).Name := 'FBoleto'+InttoStr(I);
     Titulos[I].PrepararBoleto(Tboleto(Owner.FindComponent('FBoleto'+InttoStr(I))));
//     Titulos[I].PrepararBoleto(FQuickBoleto);
     FallQuickrep.Reports.Add(TQuickRep(Owner.FindComponent('FBoleto'+InttoStr(I))));
  end;
end;

procedure TgbCobranca.Imprimir;
begin
  FAllQuickRep.Print;
  deletareports;
end;
...
begin
     ...
     for i:=0 to Titulos.Count - 1 do begin
          TBoleto.Create(Owner).Name := 'FBoleto'+InttoStr(I);
          Titulos[I].PrepararBoleto(Tboleto(Owner.FindComponent('FBoleto'+InttoStr(I))));
          FallQuickrep.Reports.Add(TqrPrinter(Owner.FindComponent('FBoleto'+InttoStr(I))));
      end;
end;
...
begin
    ...
    while titulos.count <> I do begin
       Tcomponent(Owner.components[owner.componentcount - 1]).destroy;
       Inc(i);
    end;

end;

procedure TgbCobranca.AddTitulo;
var FTitulo : TGbTitulo;
begin
