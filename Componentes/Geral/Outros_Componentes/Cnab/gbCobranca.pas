...
uses
   Windows, classes, SysUtils, Graphics, extctrls, NMSMTP,Dialogs,Printers,Quickrpt,Forms,QRPrntr, Db, DBTables
   {$IFDEF VER140}
      , Variants, MaskUtils, contnrs, gbBoleto6
   {$ELSE}
      {$IFDEF VER130}
         , Mask, contnrs, gbBoleto5,TipoDef
      {$ELSE}
         , Mask, gbBoleto4
      {$ENDIF}
   {$ENDIF}
...
type
  ...
  private
   ...
   published
      ...
      property CodigoSistema : string read fCodigo write fCodigo;
   ...
   (
      ...
      toRecebimentoInstrucaoDispensarJuros,
   ...
   private
      Report : TQuickRep;
      IndexImpressora :integer;
      fTipoOcorrencia : TTipoOcorrencia; {Tipo de ocorrência: registro de título, liquidação normal, pedido de baixa, etc...}
      fMotivoRejeicaoComando : string; {Indica porque o título / comando foi recusado. Usado apenas para receber informações do banco}
      ...
      fModelo: string;
      fEspecieStr: string;
      ftipoStr: string; {Indica quem emite o boleto: banco ou cliente}
   ...
   public
      CobrarTarifa:Boolean;
      InstrucoesFinanceiras: TStringList;
      FCobranca : TGbCobranca;
      ...
      Procedure GetInstrucoesFinanceiras; {Gera a imagem da ficha de compensação}
   published
      ...
      property EspecieStr : string read fEspecieStr write fEspecieStr;
...
{$IFNDEF VER120}
   ...
   private
      ...
      fModeloImp: string;
      fBanco: string;
      fPrimeiroSequencial: string;
      fCodConta: string;
      Procedure AgruparBoletos;
      Procedure DeletaReports;
      Procedure LocalAddReport(Sender:Tobject);
      Procedure LocalFinishedReport(Sender:Tobject);
   public
      ImprimirFormulario:Boolean;
      FlistReport : TList;
      FAllQuickRep:TQRCompositeReport;
      constructor Create(AOwner: TComponent); override;
      destructor Destroy; override;
      ...
      Function  Imprimir(FSetup:Boolean=False):Boolean;
      Procedure Vizualizar;
      Procedure AddTitulos;
   published
      property NomeArquivo   : string read fNomeArquivo write fNomeArquivo;
      property Banco         : string read fBanco write fBanco;
      property ModeloImp     : string read fModeloImp write fModeloImp;
      property CodConta      : string read fCodConta write fCodConta;
      property PrimeiroSequencial : string read fPrimeiroSequencial write fPrimeiroSequencial;
...
uses gbCob001, gbCob021, gbCob038, gbCob104, gbCob151, gbCob237, gbCob244, gbCob275, gbCob291, gbCob341, gbCob347, gbCob353, gbCob389, gbCob399, gbCob409, gbCob422, gbCob479,gbcob356;
...
begin
   ...
   InstrucoesFinanceiras := TStringList.Create;
...
begin
   ...
   Sacado.Destroy;
   Instrucoes.Destroy;
   InstrucoesFinanceiras.Destroy;
   inherited Destroy;
end;

procedure TgbTitulo.VisualizaSetup;
var
PrintDialog:TPrintDialog;
begin
{  QuickRep := TBoleto.Create(Self);
  try
    Quickrep.PrinterSetup;
    If Not Quickrep.Printer.PrinterOK then
      SysUtils.Abort;
    IndexImpressora := Quickrep.printersettings.printerindex;
  finally
   QuickRep.Free;
  end;}
...
begin
   ...
   AValorDocumento := FormatCurr('0000000000',(ValorDocumento+IIfs(CobrarTarifa,ValorDespesacobranca,0))*100); {Formata o valor com 10 dígitos, incluindo as casas decimais, mas não mostra o ponto decimal}
...
begin
   ...
   if (ValorDespesaCobranca <> 0) and CobrarTarifa then
      AInstrucoes.Add('Será Cobrada a quantia de  ' + FormatCurr('R$ #,##0.00',ValorDespesacobranca) + 'por despesas bancárias ' );
   ...
   if ValorMulta <> 0 then
      AInstrucoes.Add('Acrescer o valor do documento em  ' + FormatCurr('R$ #,##0.00',ValorMulta) + ' após o vencimento');
   ...
   begin
      ...
      txtValorDocumento.Caption := FormatCurr('#,##0.00',ValorDocumento+IIfs(CobrarTarifa,ValorDespesacobranca,0));
      ...
      txtValorDocumento2.Caption := FormatCurr('#,##0.00',ValorDocumento+IIfs(CobrarTarifa,ValorDespesacobranca,0));;
      ...
      FMascara := IIfs(Sacado.TipoInscricao = tiPessoaFisica,'!000\.000\.000\-00;0; ','!00\.000\.000\/0000\-00;0; ');
      txtSacadoCPFCGC.Caption  := IIfs(Sacado.TipoInscricao = tiPessoaFisica,'CPF: ','CNPJ: ')+ IIFs(Not Sacado.FFormata,Sacado.NumeroCPFCGC,FormatarComMascara(FMascara,Sacado.NumeroCPFCGC));
      ...
      txtValorDocumento3.Caption := FormatCurr('#,##0.00',ValorDocumento+IIfs(CobrarTarifa,ValorDespesacobranca,0));
...
{   If FCobranca <> nil then
    Fcobranca.FAllQuickRep.reports.Add(TquickRep(Aboleto));`}
...
begin
   ...
   TRY
      PrepararBoleto(ABoleto);
      ABoleto.PrinterSettings.PrinterIndex := Printer.PrinterIndex;
      ABoleto.Print;
      ABoleto.Free;
   EXCEPT
      Printer.Free;
...
Procedure TgbTitulo.GetInstrucoesFinanceiras;
begin

   if (ValorDespesaCobranca <> 0) and CobrarTarifa then
      InstrucoesFinanceiras.Add('Valor da Tarifa Bancária  ' + FormatCurr('R$ #,##0.00',ValorDespesacobranca));

   if DataProtesto <> 0 then
      InstrucoesFinanceiras.Add('Protestar em ' + FormatDateTime('dd/mm/yyyy',DataProtesto));

   if ValorAbatimento <> 0 then
      if DataAbatimento <> 0 then
         InstrucoesFinanceiras.Add('Conceder abatimento de ' + FormatCurr('R$ #,##0.00',ValorAbatimento) + ' para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataAbatimento))
      else
         InstrucoesFinanceiras.Add('Conceder abatimento de ' + FormatCurr('R$ #,##0.00',ValorAbatimento) + ' para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataVencimento));


   if ValorDesconto <> 0 then
      if DataDesconto <> 0 then
         InstrucoesFinanceiras.Add('Conceder desconto de ' + FormatCurr('R$ #,##0.00',ValorDesconto) + ' por dia de antecipação para pagamento até ' + FormatDateTime('dd/mm/yyyy',DataDesconto))
      else
         InstrucoesFinanceiras.Add('Conceder desconto de ' + FormatCurr('R$ #,##0.00',ValorDesconto) + ' por dia de antecipação');

   if ValorMulta <> 0 then
      InstrucoesFinanceiras.Add('Acrescer o valor do documento em  ' + FormatCurr('R$ #,##0.00',ValorMulta) + ' após o vencimento');

   if ValorMoraJuros <> 0 then
      if DataMoraJuros <> 0 then
         InstrucoesFinanceiras.Add('Cobrar juros de ' + FormatCurr('R$ #,##0.00',ValorMoraJuros) + ' por dia de atraso para pagamento a partir de ' + FormatDateTime('dd/mm/yyyy',DataMoraJuros))
      else
         InstrucoesFinanceiras.Add('Cobrar juros de ' + FormatCurr('R$ #,##0.00',ValorMoraJuros) + ' por dia de atraso');
...
begin
   ...
   NovoTitulo.FCobranca := Self.FCobranca;
...
begin
   ...
   Titulos.FCobranca := Self;
   FAllQuickRep := TQRCompositeReport.Create(Owner);
   FlistReport  := TList.create;
   FAllQuickRep.OnAddReports := LocalAddReport;
   FAllQuickRep.oNFinished := LocalFinishedReport;
   //   Fquickboleto := TBoleto.Create(Owner);
end;

destructor TgbCobranca.Destroy;
begin
//   FAllQuickRep.destroy;
//   Fquickboleto.Destroy;
   FlistReport.Destroy;
   Titulos.Destroy;
   inherited Destroy;
end;
      ...
      begin
         ...
         begin
            ...
            case TipoOcorrencia of
               ...
               toRecebimentoInstrucaoDispensarJuros : Add('Tipo ocorrência  : Recebimento de instrucão para dispensar juros');
...
procedure TgbCobranca.AgruparBoletos;
var i:word;
    Qry : Tquery;
begin
  for i:=0 to Titulos.Count - 1 do begin
     TBoleto.Create(Self).Name := 'FBoleto'+InttoStr(I);
     Titulos[I].PrepararBoleto(Tboleto(Self.FindComponent('FBoleto'+InttoStr(I))));
//     Titulos[I].PrepararBoleto(FQuickBoleto);
     FallQuickrep.Reports.Add(TQuickRep(Self.FindComponent('FBoleto'+InttoStr(I))));
  end;
end;

Function TgbCobranca.Imprimir(FSetup:Boolean=False):Boolean;
begin
  try
    If FSetup Then
      FAllQuickRep.PrinterSetup;
    Result := Not FSetup or (FAllQuickRep.Tag = 0);
    If Result  Then
      FAllQuickRep.Print;
  finally
    deletareports;
...
procedure TgbCobranca.Vizualizar;
begin
  Try
//    AgruparBoletos;
    FAllQuickRep.Preview;
  finally
    deletareports;
  end;  
end;
...
procedure TgbCobranca.LocalAddReport(Sender: Tobject);
var i:word;
    Qry : Tquery;
begin
     FIndex := 0;
     ...
     FallQuickrep.PrinterSettings.PaperSize := A4;
     FAllQuickRep.ReportTitle := 'Impressão de Boletos Bancários';
     FallQuickrep.Reports.Clear;
     for i:=0 to Titulos.Count - 1 do begin

            TBoleto.Create(Self).Name := 'FBoleto'+InttoStr(I);
             If Self.Findcomponent('FBoleto'+InttoStr(I)) = nil then begin
                TBoleto.Create(Nil).Name := 'FBoleto'+InttoStr(I);
             end;
//             FallQuickrep.Options := TBoleto(Titulos[0]).Options;
             Titulos[I].PrepararBoleto(Tboleto(Self.FindComponent('FBoleto'+InttoStr(I))));
             FallQuickrep.Reports.Add(Tboleto(Self.FindComponent('FBoleto'+InttoStr(I))));
      end;
end;


procedure TgbCobranca.LocalFinishedReport(Sender: Tobject);
begin
   Inc(FIndex);
   if findex < Titulos.Count then begin
     TQuickRep(Sender).QrPrinter.NewPage;
     TQuickRep(Sender).CurrentY := 1;
   end;
end;

procedure TgbCobranca.DeletaReports;
var
i : word;
begin
    i := 0;
    while titulos.count <> I do begin
       Tcomponent(Self.components[Self.componentcount - 1]).destroy;
       Inc(i);
    end;
end;
...
procedure TgbCobranca.AddTitulos;
Var
Ftitulo:TgbTitulo;
begin
    Ftitulo := TgbTitulo.Create(Self);
    FTitulos.Add(Ftitulo);
end;
