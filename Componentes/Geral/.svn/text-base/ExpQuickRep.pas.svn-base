...
uses
  ...
  QuickRpt, Db, StdCtrls, QRCtrls, QR3Const, Printers, forms;
...
type
  ...
  TQRHTMLDocumentFilter = class(TQRAbstractExportFilter)
  private
    FFreeStream : boolean;
  protected
    function GetFilterName : string; override;
...
function TQRHTMLDocumentFilter.GetFilterName : string;
begin
  Result := SqrHtmlDocument;
end;

function TQRHTMLDocumentFilter.GetDescription : string;
begin
  Result := SqrHTMLDocumentForWeb;
end;

function TQRHTMLDocumentFilter.GetExtension : string;
begin
  Result := 'HTM'; // Do not translate
end;

function TQRHTMLDocumentFilter.GetStreaming : boolean;
begin
  Result := true;
end;
...
procedure TQRHTMLDocumentFilter.CreateStream(Filename : string);
begin
  if Filename = '' then
  begin
...
procedure TQRHTMLDocumentFilter.CloseStream;
begin
  if FreeStream then
    inherited CloseStream;
end;

procedure TQRHTMLDocumentFilter.StorePage;
var
  X, Y : integer;
  Font : TFont;
  AText : string;
  Parameter : string;
begin
  writeToStream('<TABLE WIDTH=100% CELLPADDING=2 BORDER=0>');
  for Y := 1 to LineCount do
  begin
    writeToStream('<TR>');
    for X := 1 to ColCount do
    begin
      aText := GetText(X, Y, Font);
      if aText <> '' then
      begin
        Parameter := '';
        if (Font.Color <> FFont.Color) then
          Parameter := 'COLOR="#' + ColorToHtmlColor(Font.Color) + '"';
        if (Font.Size <> FFont.Size) then
          Parameter := Parameter + ' SIZE=' + HTMLFontSizeAdjust(Font.Size, FFont.Size);
        if Parameter <> '' then
          AText := '<FONT ' + Parameter + '>' + AText + '</FONT>';
      end;
      WriteToStream('<TD>' + AText + '</TD>');
    end;
    writelnToStream('</TR>');
  end;
  writeToStream('</TABLE>');
...
begin
  inherited Create(AOwner);
  QRExportFilterLibrary.AddFilter(TQRHTMLDocumentFilter);
end;

destructor TQRHTMLFilter.Destroy;
begin
  QRExportFilterLibrary.RemoveFilter(TQRHTMLDocumentFilter);
  inherited Destroy;
end;
    ...
    begin
      ...
      begin
        ...
        if TextToFloat(PChar(sCell), NotUsed, fvExtended) then
          WriteData(CellDouble, I, J, sCell)
        else
