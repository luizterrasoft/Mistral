...
type
  ...
  TRadioBControle = class(TRadioButton)
  private
...
constructor TRadioBControle.Create(AOwner: TComponent);
Var nHandle : TComponent;
begin
  nHandle := inherited Create(AOwner);
end;

destructor TRadioBControle.Destroy;
Begin
   inherited Destroy;
End;

procedure TRadioBControle.DoExit;
Var    Loop : Word;
begin
...
Procedure TRadioBControle.KeyDown(var Key:Word;Shift:TShiftSTate);
Begin
...
procedure TRadioBControle.KeyPress(var Key: Char);
Begin
...
procedure Register;
begin
  RegisterComponents('Sistema', [TRadioBControle]);
end;
