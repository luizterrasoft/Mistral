...
type
  ...
  published
    ...
    property UltColAut :Boolean Read FUltColAut Write FUltColAut;
...
var VOini,VSql,FValor,OldSql,FVl1,Fvl2,Fnome,VValor,FOrdem:String;
    ...
    FList:TRegFields;
    vwidth:integer;
    listtitlefcolor,listfontcolor:TStringList;
    listtitlecolor,listcolor:TStringList;
    f:tfont;
begin
   ...
   ListColor := TStringList.Create;
   ListFontColor := TStringList.Create;
   ListTitleColor := TStringList.Create;
   ListTitleFColor := TStringList.Create;

   for i:=0 to Columns.Count - 1 do begin
     listcolor.add(ColortoString(Columns[i].Color));
     listTitlecolor.add(ColortoString(Columns[i].Title.Color));
     listfontcolor.add(ColortoString(Columns[i].Font.Color));
     listTitleFcolor.add(ColortoString(Columns[i].Title.Font.Color));
   end;
   ...
   If DataSource.DataSet.FieldList.Count > 0 then begin
     For i:=0 to DataSource.DataSet.FieldList.Count - 1 do begin
      ...
      With Flist[Length(Flist) - 1] do begin
        ...
        if (Self.Columnbyname(Nome) <> nil) and Self.Columnbyname(Nome).Visible
           and DataSource.DataSet.FieldList[i].Visible then
            i2 := i;
...
//         If Not Fupper and ((FDadosDbGrid = nil) or Not FDadosDbGrid.CkConcatena.Checked) Then
        ...
        For i:=0 to lENGTH(Flist) - 1 do
         If FindField(Flist[i].nome) <> Nil then
          ...
          for i:=0 to listcolor.Count - 1 do begin
             Self.Columns[i].color            := StringtoColor(ListColor[i]);
             Self.Columns[i].title.color      := StringtoColor(listtitlecolor[i]);
             Self.Columns[i].Title.Font.Color := StringtoColor(listtitleFcolor[i]);
             Self.Columns[i].Font.color       := StringtoColor(listFontcolor[i]);
          end;


         if FUltColAut then begin

          for i:=0 to Self.Columns.Count - 1 do
           if Self.Columns[i].visible and
              Self.DataSource.DataSet.FieldByName(Self.columns[i].fieldname).visible then begin
               Vwidth := Vwidth + Self.Columns[i].width;
               i2 := i;
            end;
            i := Self.DataSource.DataSet.FieldByName(Self.columns[i2].fieldname).DisplayWidth;
            if vwidth + 50 < Self.Width then
              Self.Columns[i2].Width := Self.Columns[i2].Width + (Self.Width - vwidth - 50);
            Self.DataSource.DataSet.FieldByName(Self.columns[i2].fieldname).DisplayWidth := i; 
         end;
     finally
     ...
     slaux.destroy;
     ListColor.destroy;
     ListFontColor.destroy;;
     ListTitleColor.destroy;;
     ListTitleFColor.destroy;;
   end;
end;
